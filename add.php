<?php
	...
	//Получение типа страницы и загрузка в сессии для предотвращения потери значения
	if(isset($_GET['flagadd']))
		{
			$typePage = $_GET['flagadd'];
			$_SESSION['flagadd'] = $_GET['flagadd'];
		}
	else
		$typePage = $_SESSION['flagadd'];

	//Обработчик в зависимости от типа страницы
	if ($typePage == 1)
	{
...
<body>
	...
	<div class="wrapper">
		<?php
			//Меню подключается со стороннего файла
	  	...
	  	<div class="content">
	    	<section class="main">
	    		<?php
	    			...
	    			//Вывод формы добавления, в зависимости от типа страницы
	    			if ($typePage == 1)
	    			{
	    				echo'
	    				<section>
	    						...
	    						<div id="loginin">
	    							...
	    							<form action="add.php" id="registerform" method="get" name="registerform">
	    								...
	    								<p><label for="radio">Выберите тип учетной записи<br>
	    									...
	    									<input name="dol" type="radio" value="SuperUser">Суперпользователь<br>
	    									</label>
	    								</p>
	...
	<!-- Подключения скриптов и библиотек -->
